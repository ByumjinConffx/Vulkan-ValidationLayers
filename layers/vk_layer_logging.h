...
typedef enum DebugCallbackStatusBits {
    DEBUG_CALLBACK_MESSENGER = 0x00000001,  // This struct describes a VK_EXT_debug_utils callback
...
typedef struct {
    ...
    // VkDebugReportContent report related information
    ...
    // VkDebugUtilsMessengerContent messenger related information
    ...
    bool IsUtils() { return (callback_status & DEBUG_CALLBACK_MESSENGER); }
    bool IsDefault() { return (callback_status & DEBUG_CALLBACK_DEFAULT); }
    bool IsInstance() { return (callback_status & DEBUG_CALLBACK_INSTANCE); }
...
static inline void RemoveDebugUtilsCallback(debug_report_data *debug_data, std::vector<VkLayerDbgFunctionState> &callbacks,
    ...
    for (item = callbacks.begin(); item != callbacks.end(); item++) {
        if (item->IsUtils()) {
            if (item->debug_utils_callback_object == reinterpret_cast<VkDebugUtilsMessengerEXT>(callback)) break;
        } else {
            if (item->debug_report_callback_object == reinterpret_cast<VkDebugReportCallbackEXT>(callback)) break;
        }
    }
...
static inline void layer_destroy_callback(debug_report_data *debug_data, T callback, const VkAllocationCallbacks *allocator) {
    std::unique_lock<std::mutex> lock(debug_data->debug_report_mutex);
    RemoveDebugUtilsCallback(debug_data, debug_data->debug_callback_list, reinterpret_cast<uint64_t>(callback));
}
...
static inline VkResult layer_create_messenger_callback(debug_report_data *debug_data, bool default_callback,
    ...
    layer_create_callback((DEBUG_CALLBACK_MESSENGER | (default_callback ? DEBUG_CALLBACK_DEFAULT : 0)), debug_data, create_info,
                          allocator, messenger);
    return VK_SUCCESS;
}
...
static inline void ActivateInstanceDebugCallbacks(debug_report_data *debug_data, const void *pNext) {
    auto current = pNext;
    for (;;) {
        ...
        layer_create_callback((DEBUG_CALLBACK_MESSENGER | DEBUG_CALLBACK_INSTANCE), debug_data, create_info, nullptr,
                              &utils_callback);
    }
    for (;;) {
...
static inline void DeactivateInstanceDebugCallbacks(debug_report_data *debug_data, const void *pNext) {
    if (!lvl_find_in_chain<VkDebugUtilsMessengerCreateInfoEXT>(pNext) &&
        !lvl_find_in_chain<VkDebugReportCallbackCreateInfoEXT>(pNext))
        return;
